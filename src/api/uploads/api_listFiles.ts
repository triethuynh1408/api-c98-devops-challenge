            ...
            data.Contents.forEach((fileObj: any) => { // fileObj: S3.ObjectList
                if(fileObj.Size > 0) {
                    fileObjArr.push({
                        ...
                        location: `https://${process.env.AWS_BUCKET_NAME}${process.env.AWS_REGION === 'ap-southeast-1' ? '.' : '-'}s3${process.env.AWS_REGION === 'ap-northeast-1' ? '' : '-' + process.env.AWS_REGION}.amazonaws.com/${fileObj.Key}`
                    })
                }
            })
